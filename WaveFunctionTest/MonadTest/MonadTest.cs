...
namespace WaveFunctionTest.MonadTest
{
    public class MonadTest
    {
        ...
        {
            //Arrange
            IMonad<int, double> test = new Monad<int, double>(i => i / 2f);
            //Act
            test.Run(5);
        ...
        {
            //Arrange
            var test = new Maybe<int, double>(i => i / 2f);
        ...
        {
            //Arrange
            var test = new Maybe<int, double>(i => throw new Exception());
            var test2 = test.Transform(d => d.ToString(CultureInfo.InvariantCulture)) as Maybe<int, string>;
            //Act
            test2.Run(5);
            //Assert
        ...
        {
            //Arrange
            var test = new Maybe<int, double>(i => throw new ArgumentException("i ate all the cookies"));
            var test2 = test.Transform(d => d.ToString(CultureInfo.InvariantCulture)) as Maybe<int, string>;
            //Act
            test2.Run(5);
            //Assert
            ...
            Assert.That(test2.Error[1].Message, Is.EqualTo("attempting to retrieve from monad when no data is available"));
