...
namespace WaveFunctionTest.MagicSystem
{
    public class RuneTest
    {
        ...
        public void A_Gesture_Can_Record_Aspects_With_A_Quality()
        {
            ...
            g.Record(Aspect.Fortis, 3);

            Assert.That(g.Aspects[Aspect.Ignis], Is.EqualTo(0.75).Within(0.1));
            Assert.That(g.Aspects[Aspect.Fortis], Is.EqualTo(3).Within(0.1));
            Assert.That(g.Aspects.ContainsKey(Aspect.Frigus), Is.False);
        }

        [TestCase(Element.Calor, 0.75)]
        [TestCase(Element.Solidum, 3)]
        [TestCase(Element.Entropia, 0)]
        ...
        [TestCase(Element.Calor, 1)]
        [TestCase(Element.Entropia, 3)]
        ...
        public void Multiple_Runes_May_Form_An_Incantation(Element tested, double strength)
        {
            ...
            cast.Inscribe(new Gesture().Record(Aspect.Ignis).Record(Aspect.Fortis).Resolve());
            cast.Inscribe(new Gesture().Record(Aspect.Concordia, 3).Record(Aspect.Fortis).Resolve());
            Assert.That(cast.Element(tested), Is.EqualTo(strength));
        }

        [TestCase(Element.Calor, 0.5)]
        [TestCase(Element.Solidum, -0.75)]
        public void Opposite_Aspects_Affect_Eachother(Element tested, double strength)
        {
            var cast = new Incantation();
            cast.Inscribe(new Gesture().Record(Aspect.Ignis).Record(Aspect.Fortis).Resolve());
            cast.Inscribe(new Gesture().Record(Aspect.Frigus, 0.5).Record(Aspect.Debilis, 1.75).Resolve());
            Assert.That(cast.Element(tested), Is.EqualTo(strength));
        }
